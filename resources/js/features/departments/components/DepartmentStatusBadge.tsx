...
import { DEPARTMENT_STATUS_LABELS, type DepartmentStatus } from '@/types/department';

interface DepartmentStatusBadgeProps {
    /** Department lifecycle status to visualise. */
...
const STATUS_MAP: Record<DepartmentStatus, { tone: BadgeTone; dot: string }> = {
    active: { tone: 'success', dot: 'bg-success' },
    inactive: { tone: 'neutral', dot: 'bg-muted-foreground' },
};

/**
 * Compact, accessible department-status pill.
 *
...
export function DepartmentStatusBadge({
    status,
    className,
}: DepartmentStatusBadgeProps): JSX.Element {
